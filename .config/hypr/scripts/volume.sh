...
mute_status=$(echo wpctl get-volume @DEFAULT_AUDIO_SINK@ | awk '/Sink #1/{getline; print
$1}')
...
get_icon() {
    ...
    elif [[ "${current%\%}" -le 0.30 ]]; then
        echo "$iDIR/volume-low.png"
    elif [[ "${current%\%}" -le 0.60 ]]; then
...
toggle_mute() {
	if [ "$mute_status" == "false" ]; then
		 wpctl set-mute @DEFAULT_AUDIO_SINK@ 1 && notify-send -e -u low -i "$iDIR/volume-mute.png" " Mute"
	elif [ "$mute_status" == "true" ]; then
